...
#repositoryModuleFilename=$repositoryFilename.module							# REMOVE
#
#############
# Functions #
#############
...
function repositoryCreate() { # Create the sources repository
	...
	if [ $repositorySources -eq 0 ]; then
		logError "repository" "Repository could not be created. No sources has been found on this server. Check user and filesystem permissions, or maybe there is no software installed." 1
...
function collectDB2() { # Module - DB2
	...
	grep "db2ls" $repositoryFilename > $repositoryFilenameTemp
	
	while read repositoryLine; do
		echo $repositoryLine
	done < $repositoryFilenameTemp
}
...
logInfo "inventory" "Starting fewbits/ibm-inventory tool"
repositoryCreate
collectDB2
#inventoryCreate
